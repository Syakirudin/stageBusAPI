  ...
  static async createScheduleTable() {
    const query = `       
      CREATE TABLE IF NOT EXISTS schedule (
        ...
        from_stop_location_name VARCHAR(255) NOT NULL,
        from_stop_coordinates JSONB NOT NULL,
        to_stop_location_name VARCHAR(255) NOT NULL,
        to_stop_coordinates JSONB NOT NULL,
        FOREIGN KEY (from_stop_location_name, from_stop_coordinates) 
          REFERENCES stop_point(location_name, coordinates) ON DELETE CASCADE,
        FOREIGN KEY (to_stop_location_name, to_stop_coordinates) 
          REFERENCES stop_point(location_name, coordinates) ON DELETE CASCADE,
        UNIQUE (route_no, from_stop_location_name, to_stop_location_name) -- Optional unique constraint
      );
